...
model = ChatGroq(
    ...
    model_kwargs={"top_p": 0.9, "frequency_penalty": 0.2, "presence_penalty": 0.6},
)
...
def format_response(text: str) -> str:
    ...
    for para in paragraphs:
        ...
        # Numbered Lists - Only if line starts with a number
            ...
            line.strip()[0].isdigit() and line.strip()[1] == "." for line in lines
        ):
...
def conversation_chain(query: str, device_id: str):
    ...
    if context_docs:
        logger.info(
            f"Retrieved {len(context_docs)} context documents from vector search."
        )
    else:
